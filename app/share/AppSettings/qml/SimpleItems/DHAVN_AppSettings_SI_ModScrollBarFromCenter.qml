...
Item{
    ...
    function getItemX()
    {
        //console.log("rScrollCurrentValue:"+rScrollCurrentValue+ "rScrollCenterValue:"+rScrollCenterValue)
        if(rScrollCurrentValue >= rScrollCenterValue)
        {
            return Math.round(scroll.sourceSize.width / 2)
        }
        else
        {
            return Math.round(scroll.sourceSize.width / 2) -
                    Math.round((Math.abs(rScrollCurrentValue) / (rScrollValueMax - rScrollValueMin)) * (scroll.sourceSize.width))
        }
    }
    ...
    Image{
        ...
        {
            id: visibleArea
            width: Math.round((Math.abs(rScrollCurrentValue) / (rScrollValueMax - rScrollValueMin)) * (scroll.sourceSize.width))
            height: scroll.height
            anchors.top:parent.top
            anchors.left: parent.left
            anchors.leftMargin: getItemX()
            clip: true
            ...
            Image{
                ...
                source: bFocused ? HM.const_URL_IMG_GENERAL_SCREEN_SLIDER_LEFT_F : HM.const_URL_IMG_GENERAL_SCREEN_SLIDER_LEFT_N
                visible: rScrollCenterValue > rScrollCurrentValue
            }
            ...
            Image{
                ...
                source: bFocused ? HM.const_URL_IMG_GENERAL_SCREEN_SLIDER_RIGHT_F : HM.const_URL_IMG_GENERAL_SCREEN_SLIDER_RIGHT_N
                visible: rScrollCenterValue < rScrollCurrentValue
            }
            */
        }
        ...
        Item{
            id : pointerAlign
            width: Math.round(( (rScrollCurrentValue - rScrollValueMin) / (rScrollValueMax - rScrollValueMin) * (scroll.sourceSize.width) ))
        }

        Image{
            id : pointer
            source : bFocused ? HM.const_URL_IMG_GENERAL_SCREEN_POINTER_F : HM.const_URL_IMG_GENERAL_SCREEN_POINTER_N
            anchors{ verticalCenter: scroll.verticalCenter; horizontalCenter: pointerAlign.right }
        }
