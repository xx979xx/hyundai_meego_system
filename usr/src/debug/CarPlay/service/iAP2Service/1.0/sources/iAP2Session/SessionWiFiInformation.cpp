/*
    File:       SessionWiFiInformation.cpp
...
SessionWiFiInformation::SessionWiFiInformation()
{
    LOGD("SessionWiFiInformation()");
}
...
int SessionWiFiInformation::handleWiFiInformationCmd(uint32_t cmd, uint32_t handle, uint8_t* pRxData, LinkController* pLnkCtrler)
{
    int result = kIap2NoErr;
    LOGD("handleWiFiInformationCmd() cmd[%u] handle[0x%x] pRxData[%p] pLnkCtrler[%p]", cmd, handle, pRxData, pLnkCtrler);
    ...
    switch(cmd) {
        case kCmdRequestWiFiInformation : {
            ...
            LOGD("    rcv kCmdRequestWiFiInformation dummyValue[0x%x]", dummyValue);

            sendRequestWiFiInformation(pLnkCtrler);
            break;
        }
...
void SessionWiFiInformation::sendRequestWiFiInformation(LinkController * pLnkCtrler)
{
    LOGD("sendRequestWiFiInformation()");
    ...
    nEncodedDataLength = DataEncoder::encodeMsg(kIAP2PacketServiceTypeControl, kMsgIdRequestWiFiInformation, nParamIdList, paramDataList, nParamDataLenList, &encodedData);
