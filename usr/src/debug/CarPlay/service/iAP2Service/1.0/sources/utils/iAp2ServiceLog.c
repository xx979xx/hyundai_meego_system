...
#define MIBMAXLOGBUF 96
            ...
            while(loopcount > 0) {
                ...
                iAp2ServiceExternelLogWriteAddQueue( level, tmp_buf, strlen(tmp_buf) + 1);
                pbuf = pbuf + MIBMAXLOGBUF;
                loopcount--;
            }
            iAp2ServiceExternelLogWriteAddQueue( level, pbuf, strlen(pbuf) + 1);
#else
            iAp2ServiceExternelLogWriteAddQueue( level, buf, strlen(buf) + 1);
#endif		
	}	
	#else
		fprintf(stderr, "%s\n", buf);
