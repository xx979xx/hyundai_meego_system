...
FocusScope {
    ...
    x: 0; y: 0
    width: !idAppMain.generalMirrorMode ? 553+11 : 553+11+45 ; height: 532
...
//            else if(value == 0 && idRadioPresetList.requestStopSearch == true ){
//                console.log("onChangeSearchState requestStopSearch: " + idRadioPresetList.requestStopSearch + idRadioPresetList.requestStopIndex);
//                idRadioPresetList.requestStopSearch = false;
//                QmlController.setPresetIndex(QmlController.radioBand,idRadioPresetList.requestStopIndex)//QmlController.setPresetIndexFM1(idRadioPresetList.requestStopIndex);
//                QmlController.presetRecall(idRadioPresetList.requestStopIndex - 1,idRadioPresetList.channelChangedFreq); // micom has index from 0 so it need to -1
//                idChListView.currentItem.calculateFrequencyInView();
//                idRadioPresetList.requestStopIndex   = -1;
//                idRadioPresetList.channelChangedFreq = "";
//            }
...
////
    ...
    //// 2013.10.23 added by qutiguy - Update ListView data after
    Connections{
        target: UIListener
    ...
    ListView{
        ...
        onListEditChanged: {                                            // JSH 121121 , init value(Edit ON/OFF)
            ...
            if(idChListView.listEdit == true){                                       // JSH 121121 , Edit button enabled                
                ...
                //// 2013.12.03 added by qutiguy - ITS 212013 : Recover PresetList
                ///////////////////////////////////////////////////////////////////
...
//                forceActiveFocus();
    ...
    Image{ // JSH 130916
        z: -1
        //// 2014.01.06 added by qutiguy : ITS 0217691 PresetList BG Image
        x: !generalMirrorMode ? 0 : -653
        ////
        source: presetAndDialBackground == "PresetList" ? imgFolderGeneral+"bg_menu_l.png" : ""
    }
// modified by qutiguy 2014.01.22 : modified position.
    Image{
        y: -9
        x: !generalMirrorMode ? -4 : 653-695-8
        //source: showFocus && idRadioPresetList.activeFocus ? imgFolderGeneral+"bg_menu_l_s.png" : ""//imgFolderGeneral+"bg_ch_l_n.png"
    ...
    MComp.MRoundScroll{
        id: idMRoundScroll
        x: !generalMirrorMode ? 582 : 653-695+34; y: 198-systemInfo.upperAreaHeight
    ...
    MComp.MVisualCue{
        id: idMVisualCue
        x: !generalMirrorMode ? 560 : 560-662; y: 358-systemInfo.upperAreaHeight; z: 1
        //menuVisualCue: true
        arrowUpFlag: arrowUp
        arrowDownFlag: arrowDown
        arrowLeftFlag: !generalMirrorMode ? arrowLeft : arrowRight
        arrowRightFlag:!generalMirrorMode ? arrowRight: arrowLeft
    }

    /////////////////////////////////////////////////////////////////
    // idRadioPresetList functions
    ...
    function changeState(){
        ...
        if(idAppMain.globalSelectedBand =="FM1"){
            if(QmlController.radioBand != 0x01)
                return;
            if((QmlController.presetIndexFM1 -1) < 0xF0){ // JSH Jog focus
                //idAppMain.alreadySaved  = 1 // JSH 121012 (alreadySaved) not used
                idChListView.currentIndex      = QmlController.presetIndexFM1 -1
                idRadioPresetList.selectedItem = QmlController.presetIndexFM1 -1
            }
            else{
                idAppMain.alreadySaved         = 0
                idRadioPresetList.selectedItem = QmlController.presetIndexFM1
            }
        }
        else if(idAppMain.globalSelectedBand =="FM2"){
            if(QmlController.radioBand != 0x02)
                return;
            if((QmlController.presetIndexFM2 -1) < 0xF0){ // JSH Jog focus
                //idAppMain.alreadySaved  = 1 // JSH 121012 (alreadySaved) not used
                idChListView.currentIndex      = QmlController.presetIndexFM2 -1
                idRadioPresetList.selectedItem = QmlController.presetIndexFM2 -1
            }
            else{
                idAppMain.alreadySaved         = 0
                idRadioPresetList.selectedItem = QmlController.presetIndexFM2
            }
        }
        else if(idAppMain.globalSelectedBand =="AM"){
            ...
            if((QmlController.presetIndexAM -1) < 0xF0){ // JSH Jog focus
                ...
                idRadioPresetList.selectedItem = QmlController.presetIndexAM -1
            }
            else{
                idAppMain.alreadySaved         = 0
                idRadioPresetList.selectedItem = QmlController.presetIndexAM
            }
        }
        ...
        if((!idAppMain.doNotUpdate) && (idAppMain.state == "AppRadioMain" || (!idAppMain.globalMenuAnimation))){
            if(idRadioPresetList.selectedItem < 12)
                idRadioMain.jogFocusState =  "PresetList"
            else
                idRadioMain.jogFocusState =  "FrequencyDial"
        }
...
//      }
    ...
    onActiveFocusChanged: { // JSH jog focus
        console.log("========================> PresetList onActiveFocusChanged :: ",idRadioPresetList.activeFocus,idRadioMain.jogFocusState)
        if(idRadioMain.jogFocusState == "PresetList" && idAppMain.state == "AppRadioMain" && (!idRadioPresetList.activeFocus))
            focus = true;

        if(idRadioPresetList.activeFocus){
            if(!(idAppMain.presetEditEnabled || idAppMain.presetSaveEnabled)){
                idAppMain.arrowUp   = true
                idAppMain.arrowDown = false
                idAppMain.arrowLeft = false
