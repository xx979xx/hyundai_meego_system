...
MComp.MButtonHaveTicker
{
    ...
    onActiveFocusChanged: {
        if(true == idBtContactShearchDelegate.activeFocus) {
            if(0 != idBtContactSearchListView.count){
                /* 리스트로 focus가 이동하였을 때 local 팝업이 뜨면서 키패드hide 후 입력창으로 포커스 이동했을 경우,
               (검색하여 즐겨찾기 추가 시, 하나의 이름에 여러개 번호가 저장되어 있는 때)포커스 이슈가 발생하므로
               현 상황을 check할 수 있는 변수를 두어 팝업이 뜨는 시점에서 focusPoint 값을 변경하게 함 */
                listFocusOn = true;
            } else {
                listFocusOn = false;
            }
        } else {
            if(true == idQwertyKeypad.isHide() && false == idTextInputSearch.activeFocus
                    && false == idButtonSearch.activeFocus && false == idButtonBack.activeFocus) {
                listFocusOn = true;
            } else {
                listFocusOn = false;
            }
    ...
    firstTextX: 20 == gLanguage ? 512 : 101
    firstTextY: 28
    firstTextSize: 40
    firstTextWidth: 652
    firstTextHeight: 40
    ...
    firstTextBold: false //[ITS 0272238]
    ...
    secondTextX: 20 == gLanguage ? 119 : 773
    secondTextY: 28
    secondTextSize: 40
    secondTextWidth: 363
    ...
    secondTextAlies: 20 == gLanguage ? "Left" : "Right"
    secondTextElide: 20 == gLanguage ? "Left" : "Right"
        ...
        if("" == favoriteAdd) {
            ...
            if(2 > count) {
                BtCoreCtrl.invokeTrackerAddFavoriteFromSearch(first_name, last_name, formatted_name
                                                              , contact_type_1, phoneNum
                                                              , middle_name);
                //DEPRECATED BtCoreCtrl.invokeTrackerAddFavoriteFromSearch(first_name, last_name, formatted_name, contact_type_1, phoneNum )
            } else if(4 > count) {
                MOp.showPopup("popup_phonebook_search_add");
            } else {
                MOp.showPopup("popup_phonebook_search");
