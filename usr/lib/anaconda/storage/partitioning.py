...
def doAutoPartition(anaconda):
    ...
    if anaconda.id.storage.doAutoPart:
        error_msg = clearPartitions(anaconda.id.storage)
        if error_msg:
            errortxt = "\n".join(error_msg)
...
def clearPartitions(storage):
    ...
    for part in partitions:
        ...
        if part.format.type == "lvmpv" or part.format.type == "mdmember":
            errors.append(_("The device format of /dev/%s: %s is not supported.") %(part.name, part.format.type))
...
def allocatePartitions(disks, partitions, freespace):
    ...
    for _part in new_partitions:
        ...
        for _disk in req_disks:
            ...
            if _part.req_primary and new_part_type != parted.PARTITION_NORMAL:
                if (disklabel.partedDisk.primaryPartitionCount <
                    disklabel.partedDisk.maxPrimaryPartitionCount - 1):
                ...
                elif ((not extendedflag) and disklabel.partedDisk.primaryPartitionCount < disklabel.partedDisk.maxPrimaryPartitionCount):
                    new_part_type = parted.PARTITION_NORMAL
                else:
        ...
        if free is None and new_part_type is not None:
            raise PartitioningError("not enough free space on disks")
        elif new_part_type is None:
            return
