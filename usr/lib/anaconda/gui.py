...
stepToClass = {
    ...
    "timedate" : ("timedate_gui", "TimedateWindow"),
    ...
    "adduser" : ("adduser_gui", "AddUserWindow"),
    ...
    if not os.access(destDir, os.R_OK):
        try:
            ...
            window = MessageWindow("Error Saving Screenshot", 
                                   _("An error occurred copying over the "
                                   "screenshots."), type="warning")
            return
...
class InstallControlWindow:
    ...
    def setScreen (self):
        ...
        # grab the focus to input user name in the adduser screen
        # (we can only grab the focus here after all widgets added
        # in the screen.)
        if step == "adduser":
            new_screen.get_children()[1].get_children()[6].grab_focus()

        if step == "complete":
            self.mainxml.get_widget("closeButton").grab_focus()
    ...
    def run (self, runres):
        ...
        rcfile = "/usr/share/themes/Netbook/gtk-2.0/gtkrc"
