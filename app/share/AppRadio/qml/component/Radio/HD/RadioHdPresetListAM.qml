        ...
        onSignalUpdateModelData : {
            ...
            if((idAppMain.state == "AppRadioHdMain" || (!idAppMain.globalMenuAnimation)) && (!(idAppMain.presetSaveEnabled ||  idAppMain.presetEditEnabled))){
                if((prevGlobalBand  == idAppMain.globalSelectedBand) && (band  > 0x03)){ // when FG [or HD Display]
                    idRadioHdPresetListAM.currentIndex = prevPresetIndex;
                    ...
                    //    idRadioHdPresetListAM.currentIndex    = 0; // Model Changed [Index No.1 Select]
                    //idRadioHdPresetListAM.changeState();
...
//            else if(value == 0 && idRadioHdPresetListAM.requestStopSearch == true ){
//                console.log("onChangeSearchState requestStopSearch: " + idRadioHdPresetListAM.requestStopSearch + idRadioHdPresetListAM.requestStopIndex);
//                idRadioHdPresetListAM.requestStopSearch = false;
//                //QmlController.setPresetIndexFM1(idRadioHdPresetListAM.requestStopIndex); // JSH 121109 comment ==> presetRecall
//                QmlController.presetRecall(idRadioHdPresetListAM.requestStopIndex - 1,idRadioHdPresetListAM.channelChangedFreq,idRadioHdPresetListAM.channelChangedSps); // micom has index from 0 so it need to -1
//                idChListView.currentItem.calculateFrequencyInView();
//                idRadioHdPresetListAM.requestStopIndex   = -1;
//                idRadioHdPresetListAM.channelChangedFreq = "";
//                idRadioHdPresetListAM.channelChangedSps  = 0xFF;
//            }
            ...
            if(value){ // search start
                ...
                if((idRadioHdMain.jogFocusState != "PresetList") && (value == 7)){
                    //idRadioHdPresetListAM.forceActiveFocus(); // , JSH 131106 deleted
                    idRadioHdMain.jogFocusState = "PresetList";
                }
    ...
    /////////////////////////////////////////////////////////////////
    // idRadioHdPresetListAM functions
    ...
    function changeState(){
        ...
        idRadioHdPresetListAM.selectedItem = 0xFF;
                    ...
                    if(QmlController.getHDRadiobutton()){
                        if((idChListView.currentItem.presetSpsNum - 1) ==  QmlController.toFlag(QmlController.getHDRadioCurrentSPS())){
                            if(idChListView.currentItem.presetStationName == QmlController.getHDRadioSIS() && (QmlController.getRadioDisPlayType())){
                                ...
                                idRadioHdPresetListAM.selectedItem = QmlController.presetIndexAM -1;
                            }
        ...
        if((!idAppMain.doNotUpdate) && ((idAppMain.state == "AppRadioHdMain")
            ...
            //else
            if(QmlController.getRadioDisPlayType() < 2){
                if(idRadioHdPresetListAM.selectedItem < 12)
                    idRadioHdMain.jogFocusState =  "PresetList"
                else
                    idRadioHdMain.jogFocusState =  "FrequencyDial"
            }
