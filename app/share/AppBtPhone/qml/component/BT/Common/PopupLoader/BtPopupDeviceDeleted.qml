/**
 * /BT/Common/PopupLoader/BtPopupDeviceDeleted.qml
 *
 */
import QtQuick 1.1
...
MComp.MPopupTypeDim
{
    ...
    onShow: {
        ...
        qml_debug("btPhoneEnter = " + btPhoneEnter);
        qml_debug("btSettingsEnter = " + btSettingsEnter);
        /* [중국 품확 이슈] 기기 삭제 화면에서 삭제 완료 팝업 출력 시 Phone HK 입력 후, back key 동작에 삭제 화면으로 천이 하는 문제
         * 기기 삭제이며, 기기 삭제 화면 일 때, 삭제 완료 팝업 출력 시 popscreen
         */
        if("device" == delete_type) {
            if("BtDeviceDelMain" == idAppMain.state) {
                popScreen();
