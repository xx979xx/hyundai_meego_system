...
class Storage(object):
    ...
    def deviceImmutable(self, device, ignoreProtected=False):
        ...
        if not ignoreProtected and device.protected:
            ...
            return _("You cannot delete a partition of an LDL formatted "
                     "DASD.")
            ...
            for vg in self.vgs:
                if vg.dependsOn(device):
                    if vg.name is not None:
                        ...
                        return _("This device is part of an LVM volume "
                                 "group.")
    ...
    def sanityCheck(self):
        ...
        if root and root.size < 250:
            warnings.append(_("Your root partition is less than 250 "
                              "MB which is usually too small to "
        ...
        if (root and
            ...
            warnings.append(_("Your / partition is formatted as %s, "
                              "not the prefered filesystem type: %s.")
                            %(root.format.type, self.anaconda.backend.rootFsType))
        ...
        for device in self.swapDevices:
            ...
            while True:
                try:
                    ...
                    msg = _("The swap device:\n\n     %s\n\n"
                            "does not contain a supported swap volume. To "
                            "continue installation, you will need "
...
""" % time.asctime()
        ...
        for device in devices:
            ...
            #add noatime,barrier=1 to ext3 mount options
            if device.format.type == "btrfs":
                options = options + ",relatime,nodatasum"
