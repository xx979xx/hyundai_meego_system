...
// modified by Dmitry 15.05.13 updated
   function handleJogEvent( event, status )
   {
      ...
      if ( !root.focus_visible )
      {
         ...
         //return //removed for its 224786 2014.02.12
      }
...
// modified by Dmitry 15.05.13 updated
   ...
   function recheckFocusHandle()
   {
      if ( root.focus_visible )
      {
         ...
         root.hideFocus()
         //{added for Divx Button focus 2014.02.19
         if(root.name == "DivxRegistration")
         {
             root.setDefaultFocus(UIListenerEnum.JOG_DOWN)
         }
         //}added for Divx Button focus 2014.02.19
   ...
   {
      root.log( "moveFocusHandle" )
      root.log( "moveFocusHandle [" + delta_x + "][" + delta_y + "]" )
