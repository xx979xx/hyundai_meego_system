...
//}
   ...
   /* function getPropertyName(index)
    ...
    {
        var strPropertyName;
        if ( propertyModel.get(index).name.substring(0, 4) == "STR_"  )
            strPropertyName = qsTranslate(LocTrigger.empty + CONST.const_LANGCONTEXT, propertyModel.get(index).name)
        else
            strPropertyName = propertyModel.get(index).name
     ...
     {
        var strPropertyValue;
        if ( propertyModel.get(index).value.substring(0, 4) == "STR_" )
            strPropertyValue = qsTranslate(LocTrigger.empty + CONST.const_LANGCONTEXT, propertyModel.get(index).value)
        else
            strPropertyValue = propertyModel.get(index).value
...
//                    }
                ...
                DHAVN_MP_PopUp_Item_MarqueeText
                {
                    id: valueText
