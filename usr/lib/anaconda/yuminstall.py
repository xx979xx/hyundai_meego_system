...
class AnacondaYum(YumSorter):
    ...
    def readRepoConfig(self, parser, section):
        ...
        if not repo.name:
            repo.name = section
            self.logger.error(_('Repository %r is missing a name in configuration, '
                    'using id') % section)
...
""" % (anaconda.rootPath)
    ...
    def doPostSelection(self, anaconda):
        ...
        if not anaconda.id.getUpgrade():
            ...
            log.info("deselect package installer")
            self.deselectPackage("installer")
        else:
            self.ayum.update()
