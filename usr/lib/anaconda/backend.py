...
class AnacondaBackend:
    ...
    def deleteUnusedPackages(self, anaconda):
        ...
        if os.access(anaconda.rootPath + "/usr/sbin/anaconda", os.F_OK) :
            ...
            #reset rpm db
            for rpmfile in glob.glob("%s/var/lib/rpm/__db.*" % anaconda.rootPath):
                try:
                    os.unlink(rpmfile)
                except Exception, e:
                    log.debug("error %s removing file: %s" %(e,rpmfile))

            for dpackage in deletePackageList: 
                log.info("Delete package %s" %(dpackage, ))
                try:
                    buf = iutil.execWithCapture("/bin/rpm",
                                                ["-e", dpackage],
                                                stderr="/dev/tty5",
                    ...
                    log.debug("error %s deleting package: %s" %(e,dpackage))
