...
def getDefaultHostname(anaconda):
    ...
    hn = None
    bus = dbus.SystemBus()
    conn_mgr = dbus.Interface(bus.get_object(isys.CONN_SERVICE, isys.CONN_MANAGER_PATH), isys.CONN_MANAGER_IFACE)
    conn_mgr_props = conn_mgr.GetProperties()

    # XXX: account for Ip6Config objects when connman supports them
    for path in conn_mgr_props["Connections"]:
        try:
            connection = dbus.Interface(bus.get_object(isys.CONN_SERVICE, path), isys.CONN_CONNECTION_IFACE)
            connection_props = connection.GetProperties()
            ipaddr = connection_props["IPv4.Address"]
...
def _anyUsing(method):
    ...
    # The current connman only supports 'dhcp', no 'manual' supported
    # Need to rewrote the following code when connman gets ready
    if method == 'manual':
        return False
    else:
        return True
...
def hasActiveNetDev():
    try:
        ...
        conn_mgr = dbus.Interface(bus.get_object(isys.CONN_SERVICE, isys.CONN_MANAGER_PATH), isys.CONN_MANAGER_IFACE)
        conn_mgr_props = conn_mgr.GetProperties()
        if conn_mgr_props["State"] == "online":
            return True
        else:
            return False
    except:
...
class Network:
    def __init__(self):
        ...
        for dev in devhash.keys():
            ...
            # Moblin Fixme: Need to fix NM_CONTROLLED
    ...
    def write(self, instPath='', anaconda=None, username=''):
        if len(self.netdevices.values()) == 0:
            return

        if username != "":
            self.hostname = "%s-desktop" % (username,)
        else:
...
""")
    ...
    # write out current configuration state and wait for connman
    # to bring the device up, watch CONN state and return to the caller
