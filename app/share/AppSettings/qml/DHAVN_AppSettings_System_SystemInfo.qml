...
DHAVN_AppSettings_FocusedItem{
    ...
    ListModel{
        ...
        Component.onCompleted:
        {
            ...
            if(SettingsStorage.isNaviVariant)
            {
                ...
                {
                    list_model.append({"itemNum":APP.const_SETTINGS_GENERAL_ASSOCIATION, "isVisible": (SettingsStorage.currentRegion == 0),
                                          verName: QT_TR_NOOP("STR_SETTING_SYSTEM_CERT_NO"),
                                          verInfo: qsTr(SystemInfo.Association)})
                }
                else if(SettingsStorage.currentRegion == 2)
                {
                    list_model.append({"itemNum":APP.const_SETTINGS_GENERAL_ASSOCIATION, "isVisible": (SettingsStorage.currentRegion == 2),
                                          verName: QT_TR_NOOP("STR_SETTING_SYSTEM_CERT_NO_CHA"),
                                          verInfo: qsTr(SystemInfo.Association)})
                }
            }
    ...
    DHAVN_AppSettings_FocusedItem{
        ...
        Component{
            ...
            DHAVN_AppSettings_FocusedItem{
                width: 544
                height: (itemNum != APP.const_SETTINGS_GENERAL_ASSOCIATION) ?
                            (list_model.count > 5 ? 107:104) : titleText.paintedHeight + infoText.paintedHeight + 38
        ...
        DHAVN_AppSettings_System_SystemInfo_ListView{
            ...
            onContentYChanged:
            {
                if(listOfSettings.count > 6) currentTopIndex = contentY/100
                //console.log("[QML]current - cotentY:: "+ contentY)
            }
            //added for ITS 221845 systemInfo scroll issue
