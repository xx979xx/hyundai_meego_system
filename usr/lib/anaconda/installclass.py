...
class BaseInstallClass(object):
	...
	if not BETANAG:
        ...
        if flags.enablefirstboot:
            dispatch.skipStep("keyboard")
            dispatch.skipStep("timedate")
            dispatch.skipStep("adduser")
    ...
    def setDefaultPartitioning(self, storage, platform):
        # Moblin: use btrfs as the default fs
        # autorequests = [ ("/", "btrfs", 1024, None, 1, 1, 1) ]
        autorequests = [PartSpec(mountpoint="/", fstype=storage.defaultFSType,
                                 size=1024, maxSize=300*1024, grow=True,
                                 asVol=False)]

        # Moblin: create /boot partition by default 
