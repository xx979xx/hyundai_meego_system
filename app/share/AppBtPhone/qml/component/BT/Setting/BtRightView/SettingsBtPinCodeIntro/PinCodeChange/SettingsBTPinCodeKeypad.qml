...
MComp.MComponent
{
    ...
    Component.onCompleted: {
        /* 화면 초기 1회만 발생 됨*/
        idSettingsBtPinCodeKeypad.setIniNumber();
    }
    ...
    function setIniNumber() {
        ...
        if(true == idSettingsBtPinCodeKeypad.visible) {
    ...
    function savePinCode() {
        /* 입력 완료 시 동작 > 이전화면으로 이동 및 Pincode 변경 Signal 발생 */
        ...
        return lockNum
        /* 예전에는 핀코드 입력이 완료되고 이후 일정 시간 이후 화면이 전환 */
    }

    /* WIDGETS */
    Row { /* spacing 간격 만큼 가로로 자동 정렬 상단 숫자 입력 부분 */
        x: 66
        y: 37
        spacing: 7
